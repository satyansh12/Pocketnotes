...
import style from  "./MNotesTitle.module.css";
...
function MNotesTitle({ title }) {
  ...
  return (
    <div onClick={handleTitleClick} className={style.m_notes}>
      <div
        className={style.m_notes_icon}
        style={{ backgroundColor: title[0].color }}
      >
        {initials}
      </div>
      <div className={style.m_notes_title}>{newTitle}</div>
    </div>
  );
}
