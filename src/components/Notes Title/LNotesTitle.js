...
import style from './LNotesTitle.module.css';
...
function LNotesTitle({ title }) {
    ...
    return (
      <div
        ...
        className={`${style.group_title_logo} ${
          selected === title[0].name ? style.highlighted_title : " "
        }`}
      >
        <div className={style.title_logo} style={{ backgroundColor: title[0].color }}>
          {nameInitals}
        </div>
        <div className={style.group_title}>{newTitle}</div>
      </div>
    );
  }
